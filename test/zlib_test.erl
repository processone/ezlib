-module(zlib_test).
-include_lib("eunit/include/eunit.hrl").
 
-compile(export_all).
 
inflate_1024_test()->
    DeflateCmd = 1,
    InflateCmd = 2,
    %% Just random 1024 bytes data
    Data = <<"">>,
 
    case {erl_ddll:load_driver("../priv/lib", ezlib_drv), % path when using rebar2
          erl_ddll:load_driver("priv/lib", ezlib_drv)} of % path when using rebar3
            {ok, _} -> ok;
            {_, ok} -> ok;
            {{error, already_loaded}, _} -> ok;
            {ok, {error, already_loaded}} -> ok
    end,
 
    Port = open_port({spawn, ezlib_drv}, [binary]),
 
    <<0, Deflated/binary>> = port_control(Port, DeflateCmd, Data),
    ?assertMatch(<<0, _Data/binary>>, port_control(Port, InflateCmd, Deflated)).
